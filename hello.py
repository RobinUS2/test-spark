...
def fetch_lastfm_artist_image_for_udf(artist_clean_name):
  """
  Optimized fetch function for Spark UDF that uses cleaned name for caching
  but raw name for API lookup when different
  """
  # For UDF usage, we only have the cleaned name, so use it for both cache key and lookup
  return fetch_lastfm_artist_image(artist_clean_name)

def fetch_lastfm_artist_image_simple(artist_name):
  """
  Simplified fetch function for Spark UDF (no database operations)
  """
  # Last.fm API key (you should replace this with your own API key)
  api_key = "9b38e4cb99aa0de263aa854e1582fad4"
  
  if not artist_name or artist_name.strip() == "":
    return "No artist name provided"
  
  # Note: Cache checking is done outside UDF to avoid pickle issues
  print(f"🌐 Fetching from Last.fm API for artist: {artist_name}")
  ...
  try:
    ...
    if 'artist' in data and 'image' in data['artist']:
      ...
      return f"Artist not found: {artist_name}"
      
  except requests.exceptions.Timeout:
    return f"Timeout for artist: {artist_name}"
  except requests.exceptions.RequestException as e:
    return f"Request error for {artist_name}: {str(e)[:100]}"
  except json.JSONDecodeError:
    return f"JSON decode error for artist: {artist_name}"
  except Exception as e:
    return f"Error for {artist_name}: {str(e)[:100]}"
...
def fetch_lastfm_artist_image(artist_clean_name, artist_full_name=None):
    """
    Backward compatibility wrapper - returns just the image URL
    ...
    """
    image_url, _ = fetch_lastfm_artist_info(artist_clean_name, artist_full_name)
    return image_url

def populate_track_durations_from_cache():
    """Populate music_records with track durations from track_info cache"""
    print("🔄 Populating track durations from cache...")
    engine = get_database_engine()
    
    try:
        with engine.connect() as conn:
            # Check current state
            before_count = conn.execute(text("SELECT COUNT(*) FROM music_records WHERE track_duration IS NOT NULL")).scalar()
            cache_count = conn.execute(text("SELECT COUNT(*) FROM track_info WHERE duration IS NOT NULL")).scalar()
            
            print(f"📊 Current state:")
            print(f"   Records with track_duration: {before_count}")
            print(f"   Cached tracks with duration: {cache_count}")
            
            if cache_count == 0:
                print("❌ No cached track durations found!")
                return
            
            # Update music_records with cached track durations
            update_result = conn.execute(text("""
                UPDATE music_records 
                SET track_duration = (
                    SELECT ti.duration 
                    FROM track_info ti 
                    WHERE ti.artist_clean = music_records.artist_clean 
                      AND ti.song_clean = music_records.song_clean
                      AND ti.duration IS NOT NULL
                ),
                track_mbid = (
                    SELECT ti.track_mbid 
                    FROM track_info ti 
                    WHERE ti.artist_clean = music_records.artist_clean 
                      AND ti.song_clean = music_records.song_clean
                      AND ti.track_mbid IS NOT NULL
                )
                WHERE EXISTS (
                    SELECT 1 FROM track_info ti 
                    WHERE ti.artist_clean = music_records.artist_clean 
                      AND ti.song_clean = music_records.song_clean
                      AND (ti.duration IS NOT NULL OR ti.track_mbid IS NOT NULL)
                )
            """))
            ...
            # Check results
            after_count = conn.execute(text("SELECT COUNT(*) FROM music_records WHERE track_duration IS NOT NULL")).scalar()
            mbid_count = conn.execute(text("SELECT COUNT(*) FROM music_records WHERE track_mbid IS NOT NULL")).scalar()
            
            print(f"✅ Update completed:")
            print(f"   Records with duration: {before_count} → {after_count}")
            print(f"   Records with track MBID: {mbid_count}")
            
    except Exception as e:
        print(f"❌ Error populating track durations: {str(e)}")
